...
function SearchContent() {
  ...
  const initialSort = (searchParams.get('sort') || 'relevance') as 'relevance' | 'date' | 'views';
  ...
  const [sortBy, setSortBy] = useState<'relevance' | 'date' | 'views'>(initialSort);
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    ...
    if (e.key === 'ArrowDown') {
      ...
      handleSuggestionClick(
        suggestions[selectedSuggestionIndex].slug,
  ...
  const activeFilterCount =
    [selectedCategory, dateFrom, dateTo].filter(Boolean).length + (sortBy !== 'relevance' ? 1 : 0);
          ...
          <div className="absolute top-full left-0 right-0 mt-1 bg-popover border rounded-lg shadow-lg z-50 overflow-hidden">
            {suggestions.map((s, i) => (
              <button
                ...
                onMouseDown={(e) => {
                  e.preventDefault();
                  handleSuggestionClick(s.slug, s.title);
                }}
